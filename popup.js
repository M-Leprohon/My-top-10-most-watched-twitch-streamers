      ...
      xaxis: {
        ...
        labels: {
          formatter: function (value) {
            // If time is lower than 1 hour, let's just print minutes
            if(value < 1) {
              value = value.toFixed(2).toString().replace('.', 'h').replace('0h0', '').replace('0h', '') + 'min';
            } else {
              value = value.toFixed(2).toString().replace('.', 'h') + 'min';
            }
            return value;
          },
        }
